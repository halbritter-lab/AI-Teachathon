# Requirements: AI Teachathon Presentation v2
...
Requirements for presentation revision. Each maps to roadmap phases.
...
- [ ] **VIS-01**: Add branch/merge diagram to branches slide (Marp split layout)
- [ ] **VIS-02**: Add visual timeline graphic for AI revolution slide
- [ ] **VIS-03**: Capture and embed real tool screenshots (ChatGPT, Copilot in VS Code, Claude Code terminal)
- [ ] **VIS-04**: Add 5-step workflow diagram to workflow overview slide
- [ ] **VIS-05**: Store all presentation images in `docs/presentation/images/`
...
- [ ] **MARP-01**: Use `bg right/left` split layouts for slides with visuals
- [ ] **MARP-02**: Use `<!-- fit -->` for section break title auto-sizing
- [ ] **MARP-03**: Add `<style scoped>` blocks for per-slide adjustments where needed
- [ ] **MARP-04**: Differentiate section breaks visually (unique color/icon per part)
...
- **POST-03**: Gallery of completed pull requests from hands-on session

## Out of Scope

| Feature | Reason |
|---------|--------|
| KidneyQuest game screenshot | Game doesn't exist yet - built by participants during event |
| Professional illustrations | Time-constrained; diagrams and screenshots sufficient |
| Animated slides/transitions | Marp doesn't support CSS animations in exported HTML |
| Audience response system (Mentimeter etc.) | Adds external dependency; speaker-note prompts sufficient for now |
